...
//   try {
//     const response = await axios.get(VERCEL_BASE_URL + `user/${id}` ,  {
//   headers: {
...
export const userDetailsServ = async (id , token) => {
  ...
  try {
    const response = await axios.get(VERCEL_BASE_URL + `user/details/${id}`);
    return response.data;
  } catch (error) {
    console.error("loan list Error:", error);
    throw error;
...
export const userDetailsUpdateServ = async (formData , token) => {
  try {
    const response = await axios.put(VERCEL_BASE_URL + "user/update" , formData , {
    headers: {
      Authorization: `Bearer ${token}`,
